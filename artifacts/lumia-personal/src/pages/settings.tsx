...
import { Settings as SettingsIcon, Power, Save } from "lucide-react";
...
export default function Settings() {
  ...
  const handleSave = () => {
    update.mutate({
      data: { globalEnabled, idleColor, idleBrightness, idleEnabled, transitionSpeed, notificationsEnabled, overlayEnabled, overlayPort }
    }, {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="space-y-6 max-w-2xl">
        ...
        <Card className="border-border bg-card">
          <CardHeader>
            <CardTitle>Notifications & Overlay</CardTitle>
            <CardDescription>Browser notifications and OBS overlay settings.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            <div className="flex items-center justify-between py-2">
              <div>
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                <p className="font-medium">OBS Overlay</p>
                <p className="text-sm text-muted-foreground">Enable a browser source overlay for OBS.</p>
              </div>
              <Switch checked={overlayEnabled} onCheckedChange={setOverlayEnabled} />
            </div>
            {overlayEnabled && (
              <div className="grid gap-2">
                <Label>Overlay Port</Label>
                <Input
                  type="number"
                  value={overlayPort}
                  ...
                  className="max-w-xs"
                />
                <p className="text-xs text-muted-foreground">Add this as a browser source in OBS: http://localhost:{overlayPort}/overlay</p>
              </div>
            )}
          </CardContent>
        </Card>
