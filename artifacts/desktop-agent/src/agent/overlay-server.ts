...
<head>
  ...
  <style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .alert-name  { font-weight: 800; color: #fff; line-height: 1.1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .alert-msg   { color: #d1d5db; line-height: 1.3; }
  </style>
</head>
<body>
  ...
  <script>
    ...
    var DEFAULT_CFG = {
      ...
      animation: "slide", nameFontSize: 23, msgFontSize: 15, maxShown: 3,
      events: {
        follow:    { enabled:true, icon:"💚", label:"New Follower",       messageTemplate:"{username} is now following!",                accentColor:"#22c55e" },
        subscribe: { enabled:true, icon:"⭐", label:"New Subscriber",     messageTemplate:"{username} just subscribed!",                 accentColor:"#8b5cf6" },
        gift:      { enabled:true, icon:"🎁", label:"Gift Subscriptions", messageTemplate:"{username} gifted {amount} subs!",            accentColor:"#f59e0b" },
    ...
    function resolveEvent(ev) {
      var t = ev.eventType, username = ev.username, amount = ev.amount;
      var key = (t === 'subscribe' && amount > 1) ? 'gift' : t;
    ...
    function showAlert(info) {
      ...
      el.className = 'alert ' + animClass + ' entering';
      el.style.cssText = [
...
export function startOverlayServer(port: number, config?: Partial<OverlayConfig>): void {
  ...
  if (server && currentPort === port) return; // already running, config applied above
  stopOverlayServer();
